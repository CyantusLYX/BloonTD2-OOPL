...
public:
  // 建構函式
  SidebarManager(const Util::PTSDPosition &position, float height,
                 float width = 150.0f, float zIndex = 10.0f);
  ...
  std::shared_ptr<TowerButtonsPanel> getTowerButtonsPanel() const {
    return m_towerBtnPanel;
  }
