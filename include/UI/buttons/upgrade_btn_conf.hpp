...
struct UpgradeButtonConfig {
  ...
  UpgradeButtonConfig(const std::vector<std::string> &_name,
                      const std::string &_imagePath, int _cost,
...
struct TowerUpgradeConfig {
  ...
  TowerUpgradeConfig(Tower::TowerType _type,
      ...
      : type(_type),
        upgradeButtons{_upgrade1, _upgrade2} {}

  // 預設構造函數
  TowerUpgradeConfig()
      : type(Tower::TowerType::dart) {}
};
...
public:
  ...
  // 設置某類型某個升級按鈕的可用性
  static void SetUpgradeAvailability(Tower::TowerType type, int upgradeIndex,
                                     bool available);
