...
Bomb::Bomb(const Util::PTSDPosition &start, float angle, float triggerRadius,
           float explosionRadius, float maxDistance)
    : popper(start, explosionRadius), // 使用爆炸半徑作為基礎碰撞組件
      m_object(std::make_shared<Util::GameObject>(
          std::make_shared<Util::Image>(RESOURCE_DIR "/poppers/bomb.png"),
          5.0f)),
  ...
  // 創建觸發碰撞組件（較小半徑）
  m_triggerComponent =
      std::make_shared<Components::CollisionComponent>(start, triggerRadius);

  // 創建爆炸碰撞組件（較大半徑）
  m_explosionComponent =
      std::make_shared<Components::CollisionComponent>(start, explosionRadius);
...
void Bomb::move() {
  ...
  // 計算這一步移動的距離
  float step_distance = glm::distance(glm::vec2(old_pos.x, old_pos.y),
