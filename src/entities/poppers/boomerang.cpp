...
Boomerang::Boomerang(const Util::PTSDPosition &start, float angle,
                     float diameter, int life)
  ...
  m_object = std::make_shared<Util::GameObject>(
      std::make_shared<Util::Image>(RESOURCE_DIR "/poppers/boomerang.png"),
      5.0f);
  ...
  m_collisionComponent =
      std::make_shared<Components::CollisionComponent>(start, 10.0f);
...
Util::PTSDPosition Boomerang::next_position(int frames) {
  // 計算若干幀後的角度
  float futureAngle =
      m_currentAngle + m_angularSpeed * static_cast<float>(frames);
