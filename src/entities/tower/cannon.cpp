...
void Cannon::handleBloonsInRange(
  ...
  if (moveComponent) {
    ...
    Util::PTSDPosition predictedPos =
        moveComponent->next_position(static_cast<int>(bombTravelTime));
  ...
  // 創建炸彈 - 使用設定的觸發半徑和爆炸半徑
  auto bomb = std::make_shared<Bomb>(position, angle, m_triggerRadius,
                                     m_explosionRadius);
  ...
  LOG_DEBUG("CANNON: 朝 ({}, {}) 發射炸彈, 目標氣球離起點距離: {}", targetPos.x,
