...
TowerButtonsPanel::TowerButtonsPanel(const Util::PTSDPosition &position,
                                     float width, float height, float zIndex)
  ...
  LOG_DEBUG(
      "TowerButtonsPanel: Created with dimensions {}x{} at position ({}, {})",
      width, height, position.x, position.y);
}

void TowerButtonsPanel::addTowerButton(
  ...
  LOG_DEBUG("TowerButtonsPanel: Added a new button, total buttons: {}",
            m_buttons.size());
}
...
TowerButtonsPanel::getAllButtons() const {
  return m_buttons;
}
...
void TowerButtonsPanel::setPosition(const Util::PTSDPosition &position) {
  ...
  LOG_DEBUG("TowerButtonsPanel: Position updated to ({}, {})", position.x,
            position.y);
}
...
void TowerButtonsPanel::updateLayout() {
  ...
  float maxButtonWidth =
      (contentWidth - buttonSpacing * (m_buttonsPerRow - 1)) / m_buttonsPerRow;
  float buttonSize = std::min(
      maxButtonWidth, m_buttonSize.x); // 使用預設大小和最大可用大小的較小值

  // 計算按鈕行數
  size_t buttonCount = m_buttons.size();
  size_t rowCount =
      (buttonCount + m_buttonsPerRow - 1) / m_buttonsPerRow; // 向上取整

  // 檢查是否需要調整按鈕大小以適應高度
  float totalRowsHeight =
      rowCount * buttonSize + (rowCount - 1) * buttonSpacing;
  if (totalRowsHeight > contentHeight) {
    // 按鈕太大，無法適應高度，需要調整大小
    float maxButtonHeight =
        (contentHeight - buttonSpacing * (rowCount - 1)) / rowCount;
