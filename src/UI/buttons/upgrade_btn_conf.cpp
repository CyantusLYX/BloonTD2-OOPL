...
// 初始化靜態成員
std::unordered_map<Tower::TowerType, TowerUpgradeConfig> UpgradeButtonConfigManager::s_configs;
    ...
    s_configs.emplace(Tower::TowerType::dart, 
        TowerUpgradeConfig(
            ...
            UpgradeButtonConfig(
                ...
                COST_DART_RANGE_UPGRADE,
                true
            ),
            UpgradeButtonConfig(
                {"Piercing", "Darts"},
                RESOURCE_DIR "/buttons/upgrades/dart_upgrade2.png",
                COST_DART_PIERCE_UPGRADE,
                true
            )
        )
    );
...
const TowerUpgradeConfig& UpgradeButtonConfigManager::GetConfig(Tower::TowerType type) {
    auto it = s_configs.find(type);
    if (it != s_configs.end()) {
        return it->second;
    }
    
    LOG_WARN("UPGRADE_BTN_CONF: 找不到塔類型 {} 的升級配置", static_cast<int>(type));
    // 返回第一個可用的配置作為預設值
    return s_configs.begin()->second;
}

const UpgradeButtonConfig& UpgradeButtonConfigManager::GetUpgrade1Config(Tower::TowerType type) {
    return GetConfig(type).upgradeButtons[0];
}

const UpgradeButtonConfig& UpgradeButtonConfigManager::GetUpgrade2Config(Tower::TowerType type) {
    return GetConfig(type).upgradeButtons[1];
}

void UpgradeButtonConfigManager::SetUpgradeAvailability(Tower::TowerType type, int upgradeIndex, bool available) {
    auto it = s_configs.find(type);
...
void UpgradeButtonConfigManager::SetAllUpgradesAvailability(Tower::TowerType type, bool available) {
    auto it = s_configs.find(type);
    if (it != s_configs.end()) {
        ...
        LOG_WARN("UPGRADE_BTN_CONF: 找不到塔類型 {} 無法設置升級可用性", static_cast<int>(type));
    }
}

bool UpgradeButtonConfigManager::IsUpgradeAvailable(Tower::TowerType type, int upgradeIndex) {
    auto it = s_configs.find(type);
    if (it != s_configs.end() && upgradeIndex >= 0 && upgradeIndex < 2) {
