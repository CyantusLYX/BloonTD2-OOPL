...
std::pair<size_t, float> Path::findSegmentAtDistance(float distance) const {
  ...
  // 使用二分查找定位段
  auto it = std::upper_bound(m_CumulativeLengths.begin(),
                             m_CumulativeLengths.end(), distance);
...
float Path::getDistanceAtPosition(const Util::PTSDPosition &position) const {
  ...
  for (size_t i = 0; i < m_Points.size() - 1; ++i) {
    ...
    // 計算點到線段的投影位置
    float t =
        std::max(0.0f, std::min(segmentLength, dotProduct)) / segmentLength;

    glm::vec2 projection = p1 + t * segmentVector;
    float distance =
        glm::distance(glm::vec2(position.x, position.y), projection);
